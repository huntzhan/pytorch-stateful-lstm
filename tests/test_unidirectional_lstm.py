...
def test_unidirectional_single_layer_lstm():
    for lstm_cls in [UnidirectionalSingleLayerLstm, PyUnidirectionalSingleLayerLstm]:
    ...
    for lstm_cls, is_cpp in [
        ...
        for go_forward in [True, False]:
            ...
            if is_cpp:
                ...
                lstm.input_linearity_weight.data.copy_(allennlp_lstm.input_linearity.weight)
                lstm.hidden_linearity_weight.data.copy_(allennlp_lstm.state_linearity.weight)
                lstm.hidden_linearity_bias.data.copy_(allennlp_lstm.state_linearity.bias)
                lstm.proj_linearity_weight.data.copy_(allennlp_lstm.state_projection.weight)
...
def test_unidirectional_lstm():
    for lstm_cls in [UnidirectionalLstm, PyUnidirectionalLstm]:
        input_tensor = torch.rand(4, 5, 3)
        input_tensor[1, 4:, :] = 0.
        input_tensor[2, 2:, :] = 0.
        input_tensor[3, 1:, :] = 0.
